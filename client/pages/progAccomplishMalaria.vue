  ...
  <div id="viewCases">
    ...
    <div ref="content" class="viewcases-container">
      <div class="exportButtons">
        <h1 class="pageHeader"> MALARIA Program Accomplishment</h1>
      ...
      <div class="datatable">
        <div style="display: flex; justify-content: space-between; margin-top:-5px;">
          <h3 id="form-header" style="padding: 10px 0;">Barangay: ABC</h3>
        ...
        <table id="datatable">
          <thead>
            ...
            <th v-for="column in columns" :key="column" style="padding:1px;">
              <tr style="text-align: center; display: table-caption;display:grid;">
              ...
              <tr v-for="(row, rowIndex) in rows" :key="rowIndex">
                ...
                <td v-for="(cols, colIndex) in columns" :key="colIndex" style="text-align: -webkit-center; padding:0px">
                  ...
                  <span v-else-if=" (rows[rowIndex].key==='populationTotal' || rows[rowIndex].key==='populationRisk') && month>colIndex">
                    {{dataSets[colIndex][rows[rowIndex].key]}}
                  </span>
                  <tr v-else-if="month>colIndex">
                    <td v-for="(val, index) in dataSets[colIndex][rows[rowIndex].key]" :key="index"
...
export default {
  ...
  mounted() {
    ...
    this.totalPop = this.dataSets[this.month][this.rows[0].key] * 1;
    this.totalPopRisk = this.dataSets[this.month][this.rows[1].key] * 1;
  },
  methods: {
    countMonth() {
    ...
    edit(action) {
      if (action==='Cancel') {
        this.totalPop = this.dataSets[this.month][this.rows[0].key];
        this.totalPopRisk = this.dataSets[this.month][this.rows[1].key];
        this.isEdit = false;
      }
      if (action === 'Save') {
        //
        this.dataSets[this.month][this.rows[0].key] = this.totalPop;
        this.dataSets[this.month][this.rows[1].key] = this.totalPopRisk;
        this.isEdit = false;
      }
    },
    downloadPDF() {
